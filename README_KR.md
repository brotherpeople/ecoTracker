...
* $\text{Rate}_{\text{local}}$: kWh당 전력 요율. IP 위치 감지로 파악된 국가 정보 또는 수동 선택한 통화에 맞추어 `tracker/rates.json`에서 자동 로딩 (예: 한국 $150.0\text{ 원/kWh}$, 미국 $0.17\text{ 달러/kWh}$, 독일 $0.38\text{ 유로/kWh}$).
...
| 운영체제 (OS) | 실행할 설치 파일 | 실행 방법 |
| :--- | :--- | :--- |
| **Windows** | `install.bat` | 더블클릭하여 실행 (Python/pip 설치 필요) |
| **macOS** | `install.command` | Finder에서 마우스 더블클릭으로 실행 |
| **Linux** | `install.sh` | 터미널에서 `bash install.sh` 실행 |
